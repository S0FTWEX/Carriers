...
class Smscounter
{
    ...
    /**
     * Get addition to 7-bit characters map
     *
     * @return array
     */
    ...
    /**
     * Get extended 7-bit characters map
     *
     * @return array
     */
     ...
     * @return string|false if replacement string is more than 1 character in length
     */
    ...
    /**
     * Sanitize message to GSM compatible format
     *
     * @param  string $str
     * @return string
     */
    ...
    /**
     * Split SMS by character limit It will detect
     * the encoding an multipart limits to apply the split.
     *
     * @param string $str  Message text
     * @return array  Splitted messages
     */
    public function split($str)
    {
        ...
        $messages = explode('[|]', wordwrap($str, $limit-8, '[|]', true));
        $howMany = count($messages);
        
        $output = array();
        foreach ($messages as $index => $message) {
            $msgNumber = $index + 1;
            $output[] = '('.$msgNumber.'/'.$howMany.")\n".$message;
        }
